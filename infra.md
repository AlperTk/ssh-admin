...
- `pool.open(alias, timeout?)` → `{ sessionId, status, verified }` (async)
  - sessionId formatı: `<alias>-<short-hex>` (örn: `prod-server-a1b2c3d4`)
...
### Command Validation
- `command_execute` tool'unda sessionId format validation (`<alias>-<short-hex>` formatı)
...
### Session ID Formatı
- `pool.open()` → sessionId `<alias>-<short-hex>` formatında (örn: `prod-server-a1b2c3d4`)
- `src/pool.ts` → `crypto.randomUUID().slice(0, 8)` → 8 hex karakter
- `src/tools/command-tools.ts` + `file-edit-tools.ts` → regex: `^[a-zA-Z0-9_-]+-[0-9a-f]{8}$`
- Amaç: kısa, okunabilir, çarpışma riski düşük (4 trilyon kombinasyon)

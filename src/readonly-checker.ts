...
class CommandChecker {
  ...
  private hasWriteArg(cmd: string, firstToken: string): boolean {
    ...
    if ((firstToken === 'curl' || firstToken === 'wget') && (
      /\s-[oO]\s/.test(cmd) || /\s-[oO]$/.test(cmd) ||
  ...
  private validateEvalArgs(cmd: string): boolean {
  ...
  private validateExecArgs(cmd: string): boolean {
    const rest = cmd.substring(4).trimStart();
    if (!rest) return false; // exec alone = no-op, safe
  ...
  private hasWritePattern(segment: string): boolean {
    ...
    if (/\btar\b/.test(segment) && (
      /\bc[a-zA-Z]*f/.test(segment) ||
      /--create/.test(segment) ||
  ...
  private detectRedirection(s: string): boolean {
    // >> append
  ...
  private detectCommandWritePatterns(s: string): boolean {
  ...
  private detectInterpreterWrites(segment: string): boolean {
  ...
  private detectReverseShell(segment: string): boolean {
